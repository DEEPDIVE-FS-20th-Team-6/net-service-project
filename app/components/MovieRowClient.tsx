...
export default function MovieRowClient({
  ...
  // Delay the open so transitions feel like Netflix.
  ...
  const handleMouseEnter = (
    ...
    // If another overlay is open, close it first then open the next.
    if (hoverState) {
  ...
  // Close with a short exit animation.
      ...
      <div className={styles.sliderContainer} ref={sliderContainerRef}>
        ...
        <div ref={sliderRef} className={styles.slider}>
          {safeMovies.map((movie, index) => {
            const label = movie.title || movie.name || movie.original_name || '';
            ...
            return (
              <MovieRowCard
                ...
                label={label}
                isHovered={isHovered}
                onEnter={(event) => handleMouseEnter(event, movie, index, label, detailLines)}
